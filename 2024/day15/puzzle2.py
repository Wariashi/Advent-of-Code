...
class Warehouse:
    ...
    def move_robot(self, movements):
        for movement in movements:
            if movement == ">":
                ...
                if self.__can_move_north(self.robot_x, self.robot_y):
                    self.__move_north(self.robot_x, self.robot_y)
                    self.robot_y -= 1
            elif movement == "v":
                if self.__can_move_south(self.robot_x, self.robot_y):
                    self.__move_south(self.robot_x, self.robot_y)
                    self.robot_y += 1
            elif movement == "<":
    ...
    def __can_move_north(self, x, y):
        if self.tiles[x][y] == ".":
            return True
        if self.tiles[x][y] == "#":
            return False
        if self.tiles[x][y] == "@":
            return self.__can_move_north(x, y - 1)
        if self.tiles[x][y] == "[":
            return self.__can_move_north(x, y - 1) and self.__can_move_north(x + 1, y - 1)
        if self.tiles[x][y] == "]":
            return self.__can_move_north(x - 1, y - 1) and self.__can_move_north(x, y - 1)

    def __can_move_south(self, x, y):
        if self.tiles[x][y] == ".":
            return True
        if self.tiles[x][y] == "#":
            return False
        if self.tiles[x][y] == "@":
            return self.__can_move_south(x, y + 1)
        if self.tiles[x][y] == "[":
            return self.__can_move_south(x, y + 1) and self.__can_move_south(x + 1, y + 1)
        if self.tiles[x][y] == "]":
            return self.__can_move_south(x - 1, y + 1) and self.__can_move_south(x, y + 1)
    ...
    def __move_north(self, x, y):
        if self.tiles[x][y] == "@":
            self.__move_north(x, y - 1)
            self.tiles[x][y] = "."
            self.tiles[x][y - 1] = "@"
        if self.tiles[x][y] == "[":
            self.__move_north(x, y - 1)
            self.__move_north(x + 1, y - 1)
            self.tiles[x][y] = "."
            self.tiles[x][y - 1] = "["
            self.tiles[x + 1][y] = "."
            self.tiles[x + 1][y - 1] = "]"
    ...
    def __move_south(self, x, y):
        if self.tiles[x][y] == "@":
            self.__move_south(x, y + 1)
            self.tiles[x][y] = "."
            self.tiles[x][y + 1] = "@"
        if self.tiles[x][y] == "[":
            self.__move_south(x, y + 1)
            self.__move_south(x + 1, y + 1)
            self.tiles[x][y] = "."
            self.tiles[x][y + 1] = "["
            self.tiles[x + 1][y] = "."
            self.tiles[x + 1][y + 1] = "]"
