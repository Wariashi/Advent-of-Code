...
def is_solvable(result, numbers):
    ...
    numbers_after_concatenation = [int(str(numbers[0]) + str(numbers[1]))]
    for index in range(2, len(numbers)):
        numbers_after_concatenation.append(numbers[index])
    if is_solvable(result, numbers_after_concatenation):
        return True
